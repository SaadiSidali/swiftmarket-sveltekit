<script lang="ts">
	...
	const initialConfig = {
		// editorState: () => prepopulatedRichText(value),
		namespace: 'MyEditor',
		nodes: [
			HeadingNode,
			ListNode,
		...
		theme: {
			toolbar: 'flex flex-wrap bg-gray-100 p-2 border-b border-gray-300'
		}
	};
	let composer: Composer | undefined = $state();
	let editorDiv: HTMLDivElement | undefined = $state();
	let registeredUpdate = $state(false);

	onMount(() => {
		if (CAN_USE_DOM && composer) {
			const editor = composer.getEditor();
			// Initial read to set the value
			editor.update(() => {
